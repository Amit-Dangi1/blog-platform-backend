...
export class UsersService {
  ...
  async updateUserRoleByAdmin(userId:number){
   let isUser = await this.prisma.user.findUnique({where:{id:userId}});
   if(!isUser) throw new NotFoundException();

   let isUpdate = await this.prisma.user.update({where:{id:userId},data:{role:Role.Author}})
   return {UpdatedUser:isUpdate,message:"Role Updated(change into Author)"};
  }
